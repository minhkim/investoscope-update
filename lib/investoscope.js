...
export async function update(historicalQuotes) {
  ...
  for (let quote of quotes) {
    try {
      var historicalQuote = historicalQuotes[quote]
      var lastUpdatedOn = moment(historicalQuote[0].date, "YYYYMMDD")
      var today = moment().startOf('day')

      if (!lastUpdatedOn.isSame(today)) {
        debug(`!!! WARNING !!! ${quote} not updated due to outdated historical quotes.`)
      }

      var cmd = await buildAppleScriptCmd(quote, historicalQuote)
...
export async function getQuotesToUpdate() {
  return new Promise(function (resolve, reject) {
    ...
    db.all("SELECT zisin AS externalQuote, zsymbol AS symbol, zname as name FROM zquoteclient WHERE z_ent = 24;",
      function (err, rows) {
        db.close()
        ...
        if (err) {
          ...
          rows = rows.filter(function (row) {
            var externalQuote = (row.externalQuote || "").replace(/ /g, "")
            var hasExternalQuote = externalQuote.length > 1

            if (!hasExternalQuote) {
              debug(`WARNING: ${row.name} (${row.symbol}) has no ISIN set. Please set external data feed symbol in ISIN field in Investoscope if you want it to be updated.`)
            }

            return hasExternalQuote
          })

          var quotes = rows.map(function (row) { return row.externalQuote })
          resolve(quotes)
        }
    })
  })
